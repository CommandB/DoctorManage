...
protocol EvaluateMenuControllerDelegate: NSObjectProtocol {
    // MARK: 获取当前子控制器的角标
    func lxf_MenuPageCurrentSubController( index: NSInteger, menuPageController: EvaluateMenuController)
}

class EvaluateMenuController: UIViewController {
    ...
    /// 子标题按钮选中颜色
    var tipBtnHighlightedColor : UIColor = .white { didSet { updateView() } }
    /// 子标题按钮UIFont
    var tipBtnFontSize : CGFloat = 18 { didSet { updateView() } }
    /// 头部视图颜色
...
// MARK:- 更新视图
extension EvaluateMenuController {
...
extension EvaluateMenuController: UIPageViewControllerDelegate, UIPageViewControllerDataSource {
    /// 前一个控制器
