...
import Alamofire
import SwiftyJSON
class SubEvaTeachController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    var tableview = UITableView()
    var teachDataArr = [JSON]()
        ...
        NetworkTool.sharedInstance.myPostRequest(urlString,["token":UserInfo.instance().token!,"pageindex":"0","pagesize": "10","evaluatetype":"1"], method: HTTPMethod.post).responseJSON { (response) in
            ...
            case .success(let response):
                ...
                if json["code"].stringValue == "1"{
                    self.teachDataArr = json["data"].arrayValue
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if teachDataArr[indexPath.section]["items"].arrayValue.count == 0 {
            return 50
        }
        let itemCount = (teachDataArr[indexPath.section]["items"].arrayValue.count-1)/3 + 1
