...
//    [percentLabel setText:[NSString stringWithFormat:@"%i%%", percent]];
    ...
    percentLabel.text = [NSString stringWithFormat:@"%.2f分",percentLayer.percent*10];
    ...
    nameFrame.origin.y = CGRectGetMaxY(self.frame)-25;
...
-(void)setup {
    ...
    [percentLabel setFont:[UIFont systemFontOfSize:15]];
...
//    percentLabel.minimumFontSize = 10;
    ...
    self.nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, 25)];
...
- (void)setScore:(NSString *)score name:(NSString *)name animated:(BOOL)animated{
    ...
    CGFloat floatPercent = [score floatValue] / 10.0;
    floatPercent = MIN(1, MAX(0, floatPercent));
//
    percentLayer.percent = floatPercent;
    [self setNeedsLayout];
