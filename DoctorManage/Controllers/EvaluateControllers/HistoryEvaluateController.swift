//
//  HistoryEvaluateController.swift
//  DoctorManage
//
...
import SwiftyJSON
import Alamofire
class HistoryEvaluateController: UITableViewController {
    var dataSource:[JSON] = []
    var index = 0
    ...
    func setSubviews() {
        ...
        requestHistoryData(pageindex: index)
    }
    
    func requestHistoryData(pageindex:Int) {
        MBProgressHUD.showAdded(to: self.view, animated: true)
        let urlString = "http://"+Ip_port2+"doctor_train/rest/taskEvaluation/queryTeacherHistory.do"
        let params = ["pageindex":String(pageindex*10),"pagesize": "10","token":UserInfo.instance().token] as! [String:String]
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let hisEvaluateVC = HisEvaluateDetailController()
        hisEvaluateVC.headInfo = dataSource[indexPath.section]
        print(hisEvaluateVC.headInfo)
        self.navigationController?.pushViewController(hisEvaluateVC, animated: true)
    }
